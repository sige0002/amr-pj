"""Export D6.2 from the saved FreeCAD assembly. Run with FreeCAD Python.
...
def main():
    ...
    assert requirements["design_revision"] == budget["revision"] == "D6.2"
    ...
    # CAD-volume material masses, with the same catalog overrides as D6.2.
    for n, o in objects.items():
        ...
        # Added LevelBracket masses intentionally retain the D6.2 CAD ledger.
    ...
    for n, m in electrical.items():
        component("base_link", n, shapes[n], m, "D6.2 electrical catalog mass or equipment budget; uniform envelope")
    for n, o in objects.items():
    ...
    component("base_link", "wiring_allowance", box([0, 0, 128], [380, 220, 35]), .400,
              "D6.2 wiring allowance; ASSUMED distributed across first floor, not copper-fill CAD density")
    component("base_link", "battery_belt_pads", box([-146, 0, 150], [120, 30, 85]), .150,
              "D6.2 125g battery belt plus 25g pads, simplified distribution")
    component("base_link", "computer_belts_pads", box([0, 10, 130], [120, 100, 40]), .030,
              "D6.2 computer/supervisor retaining allowance")
    ...
    component("base_link", "inherited_strap_allowance", box([-136.5, 0, 150], [113, 75, 20]), .040,
              "Legacy strap allowance retained in D6.2 ledger; no new additional mass")
    component("base_link", "cargo_belts_pads", box([0, 0, 243], [200, 180, 20]), .280,
              "D6.2 cargo belts/pads allowance stowed on deck in empty variant")
    ...
    def export_mesh(link, material, items, suffix=""):
        ...
        with path.open("wb") as out:
            out.write(b"AMR D6.2 visual; METERS; FreeCAD tessellation".ljust(80, b" "))
    ...
    variants = {}
    for payload in [0, 10]:
        robot = ET.Element("robot", name="amr_d62" + ("_payload_10kg" if payload else ""))
        robot.append(ET.Comment("Generated from D6.2. SI units; +X forward, +Y left, +Z up. Inertias are estimates."))
        ...
        if payload:
            ...
            comp["base_link"].append(dict(name="cargo_belts_pads", basis="D6.2 loaded restraint allowance at CAD z310mm",
    ...
    settings = dict(schema_version=1, design_revision="D6.2", target_isaac_sim="5.0 / 5.1",
    ...
    manifest = dict(schema_version=1, design_revision="D6.2", source_cad=SOURCE.relative_to(ROOT).as_posix(),
        ...
        mass_note="Totals reconcile exactly with D6.2 ledger. Component densities, rotor/stator split, caster distribution, unselected electronics and allowances are estimates. CAD-derived tensor is not a measured tensor.",
