"""Export D6.1 from the saved FreeCAD assembly. Run with FreeCAD Python.
...
def main():
    ...
    assert requirements["design_revision"] == budget["revision"] == "D6.1"
    ...
    # CAD-volume material masses, with the same catalog overrides as D6.1.
    for n, o in objects.items():
        ...
        # Added LevelBracket masses intentionally retain the D6.1 CAD ledger.
    ...
    for n, m in electrical.items():
        component("base_link", n, shapes[n], m, "D6.1 electrical catalog mass or equipment budget; uniform envelope")
    for n, o in objects.items():
    ...
    component("base_link", "wiring_allowance", box([0, 0, 128], [380, 220, 35]), .400,
              "D6.1 wiring allowance; ASSUMED distributed across first floor, not copper-fill CAD density")
    component("base_link", "battery_belt_pads", box([-146, 0, 150], [120, 30, 85]), .150,
              "D6.1 125g battery belt plus 25g pads, simplified distribution")
    component("base_link", "computer_belts_pads", box([0, 10, 130], [120, 100, 40]), .030,
              "D6.1 computer/supervisor retaining allowance")
    ...
    component("base_link", "inherited_strap_allowance", box([-136.5, 0, 150], [113, 75, 20]), .040,
              "Legacy strap allowance retained in D6.1 ledger; no new additional mass")
    component("base_link", "cargo_belts_pads", box([0, 0, 243], [200, 180, 20]), .280,
              "D6.1 cargo belts/pads allowance stowed on deck in empty variant")
    ...
    for n in shapes:
        if n.startswith(("Rail400_", "Cross300_", "Upright3030_", "UpperRail3030_", "CustomMotorMount", "PrintedStop")) or n in (
                "AluminumDeckD3", "CasterAdapter", "CasterTop", *electrical, "Reserved_EmergencyStop", "BatteryBuckleEnvelope"):
    ...
    def export_mesh(link, material, items, suffix=""):
        ...
        with path.open("wb") as out:
            out.write(b"AMR D6.1 visual; METERS; FreeCAD tessellation".ljust(80, b" "))
    ...
    variants = {}
    for payload in [0, 10]:
        robot = ET.Element("robot", name="amr_d61" + ("_payload_10kg" if payload else ""))
        robot.append(ET.Comment("Generated from D6.1. SI units; +X forward, +Y left, +Z up. Inertias are estimates."))
        ...
        if payload:
            ...
            comp["base_link"].append(dict(name="cargo_belts_pads", basis="D6.1 loaded restraint allowance at CAD z310mm",
    ...
    settings = dict(schema_version=1, design_revision="D6.1", target_isaac_sim="5.0 / 5.1",
    ...
    manifest = dict(schema_version=1, design_revision="D6.1", source_cad=SOURCE.relative_to(ROOT).as_posix(),
        ...
        mass_note="Totals reconcile exactly with D6.1 ledger. Component densities, rotor/stator split, caster distribution, unselected electronics and allowances are estimates. CAD-derived tensor is not a measured tensor.",
