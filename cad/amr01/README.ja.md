...
2026-09-20 / FreeCAD 1.1.3 / v0.4.0。**部品を選定して組付けを検討する第一案。購入品の未公開寸法、連続運転、重量目標は未解決で、製作図の確定版ではない。**
...
- 総質量10kg・5度の従来計算における余裕込み0.506N·mを、無負荷点と拘束点の直線補間に当てはめると約73.5rpm・1.11A。これは選定の粗い見当で、連続運転・温度・低電圧性能の保証ではない。

まず水平な硬い床の低速試験用。モータードライバの電流制限を設け、温度・電流・実回転数・継手の滑りを測定する。段差・坂道・アーム操作の運転条件はこのCADでは確定していない。

## 重量の未達

メーカー・販売者の公称質量と、加工板等のCAD体積からの概算を合わせると、**骨格約4.73kg**。従来の電池・電装・計算機・配線/ガードの予算2.05kgを加えると**約6.78kg**で、台車6kg目標を約0.78kg超える。

6kg目標は変更していない。重量余裕を含めずとも未達なので、この案を「6kgで完成できる」とはしない。主な次の検討対象は300g/個の金属車輪と共用板、電池・計算機の具体選定。軽量車輪へ替える場合もハブ互換性と車輪荷重の確認が必要。従来の5/6kg重心計算は仮定例として残し、本CADの完成機を評価した計算とは扱わない。
...
ねじを除く公称モデルの干渉検査であり、全ねじ締結の強度、現物公差、キャスター全旋回の厳密干渉、ガード、ケーブル、たわみ、耐久・実機制動を検証した結果ではない。重量目標の判定は`target_met: false`として保存する。
