"""A4 electrical packaging reservations over the physical M0601C_111 chassis.
...
def planning_report(cfg):
    ...
    for name, entry in routes.items():
        ...
        if entry.get('fixed_supplied_harness'):
            ...
            retained = budget.get('rigid_source_lead_retained_mm', 0)
            total_used = plan['rounded_length_mm'] + retained
            item.update({'published_total_nominal_mm': budget['nominal_total'],
                         'published_total_minimum_mm': budget['minimum_total'],
                         'retained_source_lead_allowance_mm': retained,
                         'route_plus_retained_lead_mm': total_used,
                         'remaining_to_minimum_mm': budget['minimum_total'] - total_used,
                         'remaining_to_nominal_mm': budget['nominal_total'] - total_used,
                         'centreline_within_minimum_total': total_used <= budget['minimum_total'],
...
def build_layout():
    ...
    doc.Label = 'A4 electrical packaging STUDY | M0601C_111 | reference zones/routes'
    groups, parts = {}, []
    V = App.Vector
    ...
    for source in base.Objects:
        ...
        obj = add(source.Name, source.Shape.copy(), 'Chassis', color,
                  ...
                  role='copied_A4_physical_geometry')
        obj.Label = source.Label
        parts.append(obj)
    if not parts or doc.getObject('FrontElectronicsTrayPLA') is None:
        raise ValueError('A4 physical chassis/front electronics tray was not found')
    ...
    results = {
        ...
        'not_checked': [
            ...
            'M0601 supplied connector bodies, bundle diameter, minimum bend radius and installation slack',
    ...
    if not passed:
        raise AssertionError('A4 electrical packaging has unresolved geometry checks; see electrical_layout_validation.json')
    return results
