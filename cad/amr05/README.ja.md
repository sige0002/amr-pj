...
2026-09-21。**M0601C_111を採用し、公開ブラケットのモーター受けを参照した低背の一体金属金具へ設計を変更した。** 通常の既製金具を使う案もCADに配置したが、現フレームでは干渉する。専用品の実STEP・PDFをJLCCNCへ投入し、[2個と送料81.73 USDのサイト自動見積](MACHINING_QUOTE.ja.md)を取得した。加工先の正式審査・現物のはめあい・実荷重の検証は未完了。
...
駆動タイヤとキャスターの**接地面はどちらもz=0**。駆動輪の軸高さ50.35mm、キャスターの軸高さ25mmで、軸同士を同じ高さにする構成ではない。キャスター取付高さ65mm＋取付板4mm＝フレーム下面69mmとして揃えている。荷重によるたわみ・タイヤ摩耗・製作公差は、この公称CADの一致に含まない。
...
専用金具2個の材料込み加工は74.50 USD、日本宛OCS送料表示は7.23 USD、計81.73 USDの自動見積を別通貨で計上した（製造10日）。正式審査後の加工費、税・住所別送料・円決済額、Taobao納入諸費は未確定。共通のキャスター板/ガセットを自加工する条件であり、これらも外注する場合は別の見積項目が加わる。電池・主計算機・電源保護・車内配線等は別予算。旧取付材2,800円と旧未見積加工枠6,000円は重複計上していない。[実見積の条件と証拠画面](MACHINING_QUOTE.ja.md)
