# D6.7 BOM：費用の内訳と購入リスト

**[全明細をMarkdownで読む](BOM-details.ja.md) ／ [CSVを開く・保存する](BOM.csv)**

**途中小計 約102,882円 ＋ 未計上分**。通常積載目標10kg、車体推計10.927kg（10kgは目安）の現行構成。
...
| 部位 | 金額の目安 | 割合 |
|---|---:|---:|
| 電池・充電器・アダプター | 23,380円 | 22.7% |
| フレーム・締結材 | 18,015円 | 17.5% |
| モーター2個 | 14,000円 | 13.6% |
| 専用モーター金具2個 | 12,935円 | 12.6% |
| タイヤ・キャスター | 10,413円 | 10.1% |
| 制御・電源保護 | 9,443円 | 9.2% |
| アルミ天板 | 6,078円 | 5.9% |
| PLA・ベルト・保護材 | 3,761円 | 3.7% |
| 開閉天板の金具 | 2,871円 | 2.8% |
| その他送料 | 1,985円 | 1.9% |
| **途中小計** | **102,882円** | **100%** |

割合の分母は計上済み小計。未計上品を0円とは扱わない。円換算は記録済みの1 USD＝157.2672円（2026-09-21参考値）で、現在の決済レートではない。元の小計は83,868円＋120.90 USD。行ごとの四捨五入で端数差が出る。
...
価格記録は既存の販売ページ確認・引継価格。自動見積はJLCCNCの実サイト記録で、未発注・担当者審査前。**モーター14,000円は仮予算**。PLA2282円は材料消費参考で、新規スプールの購入額ではない。
...
| [溝ナット HNTT6-6](https://www.amazon.co.jp/dp/B0DK9C99YH)<br>[F04・仕様・備考](BOM-details.ja.md#item-f04) | 78個／100個 | 5,112円 | 価格記録 |
...
| [ブラケット・天板固定ボルト](https://www.amazon.co.jp/dp/B01N3BIIMO)<br>[D3_M6・仕様・備考](BOM-details.ja.md#item-d3_m6) | 56個／70個 | 2,360円 | 価格記録 |
| 印刷ストッパ固定ねじ<br>[D3_STOP_BOLTS・仕様・備考](BOM-details.ja.md#item-d3_stop_bolts) | 8個／未定 | 未計上 | 未計上 |
| [ストッパ・床継ぎ目用ナット](https://www.amazon.co.jp/dp/B0BVQJS1D2)<br>[D3_STOP_NUTS・仕様・備考](BOM-details.ja.md#item-d3_stop_nuts) | 32個／40個 | 546円 | 価格記録 |
| ストッパ用座金<br>[D3_STOP_WASHERS・仕様・備考](BOM-details.ja.md#item-d3_stop_washers) | 8枚／未定 | 未計上 | 未計上 |
| [床継ぎ目・ケース蓋・天板ヒンジのM4×16](https://store.onokatsu.co.jp/products/133-00-04)<br>[D62_FLOOR_SCREWS・仕様・備考](BOM-details.ja.md#item-d62_floor_screws) | 24個／60個 | 770円 | 価格記録 |
| 継ぎ目支持梁端の大径平座金<br>[D62_BEAM_WASHERS・仕様・備考](BOM-details.ja.md#item-d62_beam_washers) | 4個／未定 | 未計上 | 未計上 |
| [床・ケース・ヒンジのM4 OD12平座金](https://www.amazon.co.jp/dp/B0H8RT271F)<br>[D64_FLOOR_WASHERS・仕様・備考](BOM-details.ja.md#item-d64_floor_washers) | 28枚／40枚 | 860円 | 価格記録 |
...
| [手動ARMボタン（黒）](https://www.amazon.co.jp/dp/B075SQ41JN)<br>[ELEC_ARM・仕様・備考](BOM-details.ja.md#item-elec_arm) | 1個／1個 | 835円 | 価格記録 |
| [全電源の主スイッチ](https://www.amazon.co.jp/dp/B075SW22KP)<br>[ELEC_MAIN・仕様・備考](BOM-details.ja.md#item-elec_main) | 1個／1個 | 621円 | 価格記録 |
| [操作部・ヒンジ・ロックのM6 OD18×t1平座金](https://www.amazon.co.jp/dp/B0DZVCSRXG)<br>[ELEC_CASE_WASHER・仕様・備考](BOM-details.ja.md#item-elec_case_washer) | 18個／30個 | 968円 | 価格記録 |
...
| 品目 | 使用／購入 | 金額 | 根拠 |
|---|---|---:|---|
| 床・PCカバー・ストッパ・操作ケース・ヒンジアダプター<br>[P04・仕様・備考](BOM-details.ja.md#item-p04) | 消費1141g | 2,282円 | 材料消費 |
...
| 品目 | 使用／購入 | 金額 | 根拠 |
|---|---|---:|---|
| [トルクヒンジ](https://www.amazon.co.jp/dp/B007628W3W)<br>[HATCH_HINGES・仕様・備考](BOM-details.ja.md#item-hatch_hinges) | 2個／2個 | 2,344円 | 価格記録 |
| [天板ロック用M6蝶ボルト](https://www.amazon.co.jp/dp/B002A5PVN4)<br>[HATCH_LOCKS・仕様・備考](BOM-details.ja.md#item-hatch_locks) | 2個／18個 | 527円 | 価格記録 |
...
| [電源着脱コネクタ・ヒューズ等の配電部品](BOM-details.ja.md#item-u06) | 走行台車／未選定 |
