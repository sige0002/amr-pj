#!/usr/bin/env python3
"""Reproducible linear S6 shell screen of D2, using Triangle and CalculiX.
...
def generate(directory,area,ncircle,triangle):
    cfg=json.loads((HERE.parent/'deck_parameters.json').read_text())
    g=cfg['geometry'];s=cfg['structural_screening']
    assert s['minimum_thickness_for_screening_mm']==3.9 and s['youngs_modulus_MPa']==69000
    assert g['plate_LWT_mm']==[300,300,4] and g['support_block_LWT_mm']==[20,15,15]
    (directory/'input_parameters.json').write_bytes((HERE.parent/'deck_parameters.json').read_bytes())
    ...
    for x in g['grid_coordinates_from_center_mm']:
        for y in g['grid_coordinates_from_center_mm']:circle(x,y,2.25)
    ...
    with inp.open('w') as f:
        f.write('*HEADING\nD2 minimum3.9mm,15kg cargo+1.6kg dead+80N strap equivalent,linear\n*NODE,NSET=ALLN\n')
    ...
    meta={'nodes':n,'S6_elements':len(elems),'maximum_triangle_area_mm2':area,'circle_segments':ncircle,
          'mesh_plan_area_mm2':float(areas.sum()),'CAD_plan_area_mm2':json.loads((HERE/'geometry.json').read_text())[0]['volume_mm3']/4,
          ...
          'deck_parameters_sha256':sha(HERE.parent/'deck_parameters.json')}
...
def post(directory,meta,xy,elems,pads):
    ...
    result=dict(meta,max_downward_deflection_mm=float(-uz.min()),max_upward_deflection_mm=float(uz.max()),
    ...
    ax.set(aspect='equal',xlabel='X [mm]',ylabel='Y [mm]',title='D2 linear shell: downward displacement [mm]\n15 kg + 1.6 kg + 80 N; t = 3.9 mm; six ideal support patches')
