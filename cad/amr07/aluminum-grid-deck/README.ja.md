# D2：格子穴アルミ荷台（支持構成見直し中）

> **最新の支持変更は[P1：溝付き3030＋既製ブラケット](../printed-deck-frame/README.ja.md)へ反映済み。** P1のCAD・画像・BOM・検査結果を別にまとめた。以下は旧構成の履歴。

> **2026-09-22：支持構成を見直し中。** ユーザーが角棒の切断・穴あけを伴う支持案を不採用としたため、D2の支持ブロック・ストッパーも置換対象。以下は変更前の板・支持条件に対する見積と解析の記録であり、置換後の購入仕様ではない。
