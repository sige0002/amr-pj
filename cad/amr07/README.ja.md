...
**通常積載10kg、5°登坂時8kg、構造検証は荷物15kg・静的安全率2。車体は電池・電装・荷台込み10kg以下を目標とする。** 25kgは構造検証時の総重量で、荷物25kgを載せる設計ではない。今回は金具の機能公差・ねじ座面・ナットと溝の実寸、荷台と固定具、荷物CG範囲、停止回路構成と試験仕様を修正した。

3担当によるレビューを反映したが、**実機のSF2成立、低速連続出力、無励磁保持は未確認**。CADの干渉検査と条件付き計算の完了を、製作・積載走行の承認とはしない。[指摘ごとの対応と残る確認](REVIEW_RESPONSE.ja.md)。
...
半透明の荷物・電装・電池は予約形状。ベルトも経路例で、完成品の詳細形状ではない。電装予約は既存ねじ頭と荷台を避けたが、取付台・筐体・新しい実配線の設計は残る。無励磁での坂道保持は未設計なので、8kg/5°は現時点では駆動の設計条件である。
