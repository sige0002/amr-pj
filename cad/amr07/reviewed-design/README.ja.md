...
**公開寸法の照合追補**：[寸法根拠と修正点](SWITCH-DIMENSIONS.ja.md)。3214の仮円柱をメーカー外形19.5×30.5mm・前方高さ21.5mmの包絡形状へ修正。HWの接点/レバー包絡と回り止め切欠き寸法も訂正した。
...
- 250型端子とHW配線の確保空間を色分けした。主電源の接続メス端子/完成リードは型番未選定。HW端子中心座標は参照カタログに寸法記載がない。確保空間を実部品の確定形状として扱わない。
...
ナットの持続締付を受けるPLAは、押下150Nの梁計算とは別にクリープ・回転解除・繰返し押下を実機確認する。指定締付を勝手に下げて合格扱いにしない。主電源ナットの未掲載寸法、未選定の接続端子、非常停止の実負荷・突入適合は別々の未解消事項として残す。公開済みの外形まで現物待ちとはしない。

## 確認結果と費用

車体推計**10.363kg**（E3比約93g減、実測前）。通常積載10kg・静的比較15kg/SF2の条件を維持する。標準PLAは12部品。

変更部9,018組で新規干渉なし。天板・電池取り外し、上方/後方の非常停止操作、主電源操作、ナット工具、蓋50mm取外しを確認した。蓋は5mm刻み、ハーネス移動は2mm刻みの検査であり、実配線の連続変形や曲げを検証したものではない。スイッチ蓋の整備は電池を外す条件。
