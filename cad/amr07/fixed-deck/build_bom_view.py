...
groups=[('電池・充電器・アダプター',['POWER_KIT','POWER_KIT_SHIP','POWER_ADAPTER','POWER_ADAPTER_SHIP']),('モーター',['D01']),('モーター専用金具',['Q01','S08']),('タイヤ・キャスター',['D02','C01','P01','S02','S06']),('天板（固定ボルトは共通締結材）',['DECK_plate','DECK_CNC_SHIP']),('操作部・USB線（電装主要部は未計上）',['E02']+[x['ID'] for x in rows if x['ID'].startswith('ELEC_')]),('PLA・ベルト',['P04','H02','DECK_cargo_straps','DECK_edge_protection_and_slack_retention','POWER_PADS','POWER_LOW_STRAPS'])]
...
lines=['# D6.9 BOM（電装製作条件E1を反映）','',
       ...
       f'円換算は記録済みの2026-09-21参考値1 USD＝{rate:.4f}円。現在の決済額ではありません。購入パック全額を計上し、余りを按分していません。PC・その電源・完成済み電装・代替非常停止・加工済み配線などは未計上、モーター14,000円は仮予算、PLAは材料消費の参考です。','',
...
lines=['# D6.9 BOM 全明細（電装製作条件E1を反映）','',
       '**完成済み電装と配線は未選定です。** [製作条件と旧部品の撤回](../electrical-buildability/README.ja.md)。','',
