...
groups=[('電池・充電器・アダプター',['POWER_KIT','POWER_KIT_SHIP','POWER_ADAPTER','POWER_ADAPTER_SHIP']),('モーター',['D01']),('モーター専用金具',['Q01','S08']),('タイヤ・キャスター',['D02','C01','P01','S02','S06']),('天板（固定ボルトは共通締結材）',['DECK_plate','DECK_CNC_SHIP']),('制御・電源',['E02']+[x['ID'] for x in rows if x['ID'].startswith('ELEC_')]),('PLA・ベルト',['P04','H02','DECK_cargo_straps','DECK_edge_protection_and_slack_retention','POWER_PADS','POWER_LOW_STRAPS'])]
...
lines=['# D6.9 BOM','',f'**途中小計 {total:,.0f}円相当＋未計上分**。原通貨：{c["subtotal"]["JPY"]:,.0f}円＋{c["subtotal"]["USD"]:.2f} USD。','', '[全明細Markdown](BOM-details.ja.md) ／ [CSV](BOM.csv) ／ [加工見積の証拠](MACHINING_QUOTE.ja.md)','',f'円換算は記録済みの2026-09-21参考値1 USD＝{rate:.4f}円。現在の決済額ではありません。購入パック全額を計上し、余りを按分していません。PC・その電源・専用基板・配線などは未計上、モーター14,000円は仮予算、PLAは材料消費の参考です。','',f'ヒンジ案D6.8から約{-c["difference_from_D68_JPY_reference"]:,.0f}円減。ヒンジ・専用L金具・蝶ボルト等の購入を削除。M6固定6点は既計上パックの余りを使い、新規パック購入なし。天板は同一形状のD3実自動見積を再使用し、今回の価格再取得はしていません。','', '| 部位 | 円換算参考 | 割合 |','|---|---:|---:|']
...
lines=['# D6.9 BOM 全明細','','[概要・費用割合](BOM.ja.md) ／ [CSV](BOM.csv)','']
for r in rows:
