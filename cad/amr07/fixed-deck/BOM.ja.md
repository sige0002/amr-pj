# E3 BOM（Pico＋RS485基板・直接非常停止）

**通信は手持ちPico＋ユーザー提示RS485基板。非常停止はHW1B-V402Rの左右別接点。実配線・電源適合は確認待ちです。** [最小構成](../electrical-buildability/README.ja.md)。

**途中小計 94,466円相当＋未計上分**。原通貨：75,452円＋120.90 USD。

[全明細Markdown](BOM-details.ja.md) ／ [CSV](BOM.csv) ／ [加工見積の証拠](MACHINING_QUOTE.ja.md)

旧専用回路部品とはんだ端子の非常停止、計6,519円を[価格履歴](../electrical-buildability/withdrawn-parts.ja.md)へ移しました。通信基板・最終ハーネス等が未計上なので、この減額は節約ではありません。割合も完成車の費用割合ではありません。

E3でARM計835円と、追加監視等を初回必須から外しました。全電源用の主スイッチ621円は復帰しています。[条件付き・後工程の部品](../electrical-buildability/deferred-parts.ja.md)は下の必須未計上品へ混ぜていません。

円換算は記録済みの2026-09-21参考値1 USD＝157.2672円。現在の決済額ではありません。購入パック全額を計上し、余りを按分していません。車載LinuxミニPC・給電部材・Pico用RS485基板・ヒューズ・配線などは未計上、モーター14,000円とUSB線500円は仮予算、PLAは材料消費の参考です。計算機はLinuxミニPCを想定し、機種は固定しません。非常停止は2,387円＋東京向け送料780円を計上、基板のAmazon表示価格は未取得です。

機械部分はヒンジ案D6.8から約15,547円減。ヒンジ・専用L金具・蝶ボルト等の購入を削除。M6固定6点は既計上パックの余りを使い、新規パック購入なし。天板は同一形状のD3実自動見積を再使用。E3のPLA増加を反映し、既存価格は再取得していません。

| 部位 | 円換算参考 | 計上済み分の割合 |
|---|---:|---:|
| 電池・充電器・アダプター | 23,380円 | 24.7% |
| フレーム・締結材・共通送料 | 20,000円 | 21.2% |
| モーター | 14,000円 | 14.8% |
| モーター専用金具 | 12,935円 | 13.7% |
| タイヤ・キャスター | 10,413円 | 11.0% |
| 天板（固定ボルトは共通締結材） | 6,078円 | 6.4% |
| 操作部・USB線（電装主要部は未計上） | 4,288円 | 4.5% |
| PLA・ベルト | 3,371円 | 3.6% |
...
| 部品 | 使用／購入 | 原通貨金額 |
|---|---|---:|
| [LinuxミニPC–Pico用USBデータケーブル](BOM-details.ja.md#item-e02) | 1／未定 本 | 500 JPY |
| [Pico用2チャンネルRS485完成基板](BOM-details.ja.md#item-u08) | 未定／未定  | 未計上 |
| [非常停止スイッチ HW1B-V402R](BOM-details.ja.md#item-u22) | 1／1 個 | 2387 JPY |
| [Picoケース固定用M3締結材](BOM-details.ja.md#item-u24) | 未定／未定  | 未計上 |
| [非常停止スイッチ送料（住所未照合）](BOM-details.ja.md#item-e3_estop_ship) | 1／1 式 | 780 JPY |
...
| 部品 | 使用／購入 | 原通貨金額 |
|---|---|---:|
| [床・PCカバー・荷物止め・操作ケース](BOM-details.ja.md#item-p04) | 946／0 g | 1892 JPY |
...
| ヒューズ・ホルダー | 走行台車／未選定 |
| Pico用2チャンネルRS485完成基板 | 完成品構成へ再選定・未計上 |
...
| 荷物容器・敷板 | 積載物／未選定 |
| Picoケース固定用M3締結材 | 必要数確定・価格未計上 |

[E3設計と組立手順](../pico-control/README.ja.md) ／ [重量・強度の比較計算](PAYLOAD_REVIEW.ja.md)
