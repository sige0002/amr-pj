...
groups = [
    ...
    ('フレーム・締結材', ['F01','F02','D6_POSTS','F03','F04','P03','H01','MW','D3_M6','D3_STOP_BOLTS','D3_STOP_NUTS','D3_STOP_WASHERS']),
...
with (HERE/'BOM-summary.csv').open('w',encoding='utf-8-sig',newline='') as f:
    w=csv.writer(f);w.writerow(['部位','円換算参考','途中小計に占める割合%'])
    for c in category_rows:w.writerow([c['category'],round(c['JPY_reference'],2),round(c['percent_of_priced_subtotal'],2)])

lines = [f'# D6.1 BOM：費用の内訳と購入リスト', '',
    f'**途中小計 約{jpy(total)} ＋ 未計上分**。通常積載目標10kg、車体推計9.895kgの現行構成。', '',
...
lines += [f'| **途中小計** | **{jpy(total)}** | **100%** |', '',
    f'割合の分母は計上済み小計。未計上品を0円とは扱わない。円換算は記録済みの1 USD＝{rate:.4f}円（{summary["FX_date"]}参考値）で、現在の決済レートではない。元の小計は73,699円＋120.90 USD。行ごとの四捨五入で端数差が出る。', '',
...
lines += ['', '価格記録は既存の販売ページ確認・引継価格。自動見積はJLCCNCの実サイト記録で、未発注・担当者審査前。**モーター14,000円は仮予算**。PLA764円は材料消費参考で、新規スプールの購入額ではない。', '',
    '## 購入リスト', '',
...
for c in category_rows:
    ...
    lines += ['', '</details>', '']
lines += ['**今回の根元補強：金具4個・溝ナット8個は購入予定パックの余りを使用。金具・ナットの追加購入額0円、M6×12ねじ8本の追加価格は未確定。**', '',
    '## これから金額が増える項目', '',
