...
| 上段支持 | 100mm溝付き3030支柱4本＋300mm上段レール2本＋HBLFSN6金具8個 |
...
| 重量 | **推計9.728kg**。10kgまで0.272kg。電池・電装・荷台込み、実測前 |
...
支柱は**切断済み100mmのSUS SF2-30・30**。上段と基礎レールの間に端面を直接当て、荷台→上段レール→支柱端面→基礎レールの短い経路で鉛直荷重を伝える。各支柱の上下にHBLFSN6を1個ずつ、反対側の面に配置し、M6×12と溝ナットで締結する。主支持にPLAや長いスペーサーを挟まない。端面タップや追加の金属穴加工も不要。
...
**片側締結のレビュー：** 鉛直部材の計算だけでは接合部の安全率を確認できない。旧簡略金具応力の比較値を撤回し、メーカーの許容荷重条件と不足する方向別評価を整理した。根元へ反対側金具4個を追加する候補は約167g増・推計9.895kgで、購入予定パックの余りを使え、配置・工具・交換経路も干渉しない。ただし左右の剛性まで確認したものではなく、現行CADへの採用は未確定。[締結レビューと補強候補の比較](JOINT_REVIEW.ja.md)
...
丸め前の台帳はJSONに保存。車体CGの差分見積は、D3の仮定を引き継ぐとX−9.0～7.9、Y−7.4～9.5、Z123.4～144.6mm。未選定機器や配線の実配置を考慮して、上記の受入範囲には余裕を取った。実測重量・重心は未確認で、10kgまでの余裕も大きくはない。

## 費用と成果物

追加購入の表示額は、[100mm支柱4本](https://www.amazon.co.jp/dp/B0CB5N6ZJJ)495円、[HBLFSN6の追加10個パック](https://www.amazon.co.jp/dp/B0DKF1FCD6)917円、[HNTT6-6の追加50個パック](https://www.amazon.co.jp/dp/B0DK9C99YH)2,556円、計3,968円。使用金具は全車16個、ナット66個なので、購入後余りはそれぞれ4個・34個。追加M6×12は16本、価格未確定。表示価格と未確定分を分けた。[商品観測記録](purchase_observations.json)
...
- [全車BOM CSV](BOM.csv)／[費用集計](cost_summary.json)
