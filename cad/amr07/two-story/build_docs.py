...
mass=p['vehicle_mass_estimate_kg'];delta=p['barrier_increase_from_D64_kg']
delta_word=f"約{abs(delta)*1000:.0f}g{'減' if delta<0 else '増'}"
grams=sum(x['solid_mass_g'] for x in prints)
filament_delta=c['barrier_change_from_D64']['PLA_material_reference_increase_JPY']
...
def components():
    names={'retained_lower_structure_drive_and_hardware':'下段骨格・足回り・既存締結材',
        ...
        'new_floor_hardware':'床・支持梁の金属締結材（現行D6.5）','all_PLA':'PLA床・支持梁・PCカバー・荷物ストッパ',
...
payload=f'''# D6.5：車体重量と積載

**車体は推計{mass:.3f}kg、前版D6.4から{delta_word}。通常積載の設計目標10kgを維持する。** ユーザーの指示により、車体10kgは厳密な上限から目安へ変更した。必要な支持を省いて10kgへ合わせない。
...
floor=f'''# D6.5：通常ねじ＋大径座金を計算機側から覆う
...
PC固定部は、床と一体の10×10×5.6mm受け4個（中心X±50・Y±40）→厚さ2mmの連続PLAカバー→1mmパッド→ケースの順。PC下面は110mm、カバー下面107mm・上面109mm。金属ねじ上端106.4mmとの公称隙間は0.6mmだが、接触防止を隙間だけに頼らず2mmの板を間に置く。カバー上面に締結金属や通し穴を出さない。

**裸基板をこのパッドへ直接載せたり、基板へベルトを掛けたりする設計ではない。** 機種選定後に、ケースまたは取付穴に合う絶縁スペーサーと裏面部品の逃げを決める。底面吸気を塞ぐ機種は、この配置のまま採用しない。[カバーの構造・検査・未確定事項](COMPUTER_BARRIER.ja.md)

![カバーを取り付けた状態](cad-screen-pc-isolation.png)

![カバーを外すと床の金属ねじへアクセス](cad-screen-pc-isolation-exploded.png)
...
2. 基礎フレームのHBLFSN6金具8個を各2本のM6で締める。4枚の床を載せ、各板のM6を3本、中央のM4通常ねじを1本、上下のOD12座金各1枚と共に取り付ける。M4は上から3mm六角レンチ、下から7mmナット用ソケットを入れる。PCとカバーを載せる前に締める。
3. 連続PLAカバー・パッド・ケース入り機器・ベルトを取り付ける。梁のM6を再調整する場合は床とM4を先に外す。

PLAを挟む部分へ金属同士と同じM6締付トルクを使わない。締付けと緩みの保持は現物で確認する。CADでは上記工程の梁ねじ用ドライバー4経路とM4ナット用ソケット4経路も検査した。

印刷対象は床4枚＋梁1個＋カバー1個。最大229.8×149.8×26mmでP1Sの256mm範囲に収まる。6部品の中実CAD体積換算は約{grams:.0f}g。荷物ストッパ4個はD3のSTLを継承する。STLは配置座標から原点移動したデータで、印刷姿勢は未設定。床はリブを下にしてサポートを使うか、中央継ぎ目を下に立ててブリムと位置決め壁の局所サポートを検討する。梁は広い上面をベッド側とし、ベルト溝のブリッジを確認する。実スライス・サポート量・印刷試験は未完了。
...
実CADの床4枚＋支持梁について、電装2.4kg・カバー自重約39g・床と梁の自重を与えた二次四面体解析も追加した。100%充填の均質体・E=1000MPaを仮定し、機器を10mm角の脚4個で受ける場合、最大たわみは重力だけで{pla_feet[0]['max_down_mm']:.2f}mm、ベルト張力10N／脚で{pla_feet[10]['max_down_mm']:.2f}mm、30N／脚で{pla_feet[30]['max_down_mm']:.2f}mm。粗細2メッシュを比較し、荷重と反力のつり合いを検査した。局所応力は収束未確認。[計算条件・応力・ねじ締付け・改善点](pla-strength/README.ja.md)
...
前版D6.4との重量差は{delta_word}。PLA材料消費の参考増額は{filament_delta:,}円（2円/gの既存基準）。今回の金具・ねじの追加は0個で、金属切断・穴加工・CNC見積は発生しない。機種別のケースや基板固定具は未選定・未計上。四隅の金具廃止と410円の計上減は過去のD6.2→D6.3の変更で、今回の差額へ重ねて差し引かない。
...
readme=f'''# D6.5：計算機と床の金属ねじをPLAカバーで隔てる

**中央の金属ねじ・座金を、貫通穴のない厚さ2mmのPLAカバーで覆った。** PCの機種未選定なのに、底面の平らなケースだけを想定していたD6.4の不足を修正する。ケースの座面は108→110mm、カバー約39gを追加。車体推計{mass:.3f}kg、前版から{delta_word}。裸基板の固定は、機種別ケースまたは取付穴対応の絶縁スペーサーで具体化する。

![金属ねじを覆う連続PLAカバー](cad-screen-pc-isolation.png)
...
| PC | 120×100×60mm・0.5kgの予約、座面110mm、硬い受け4か所＋2mm連続カバー＋1mmパッド、上15mm通風・横30mmコネクタ空間 |
...
{v['physical_parts']}物理部品・{v['checked_physical_pairs']:,}組の体積干渉、予約機器、連続交換包絡、キャスター旋回、タイヤの外向き抜出し、36格子穴、支柱金具の工具経路を検査した。保存FCStd・STEPの体積と形状を再照合し、印刷6部品の閉じたSTLを確認した。床固定の座面と工具経路も別スクリプトで検査する。電池交換上端と荷台裏締結部の余裕は16.3mm。
...
1階PLAは実形状5部品の解析を追加した。電装2.4kg＋カバー約39g＋床・梁自重・100%充填相当・E=1000MPaの仮定で、脚4個の機器を載せた最大たわみは{pla_feet[0]['max_down_mm']:.2f}mm、ベルト10N／脚では{pla_feet[10]['max_down_mm']:.2f}mm。皿穴をなくして全厚2.4mmを残し、座金の支持面積を増やした。ベルト反力の課題は残る。局所応力の収束、締付けと長期保持は未確認。[PLA計算書・解析画像](pla-strength/README.ja.md)
...
corner_text=f'''# 四隅の固定：D6.3で変更、D6.5でも維持
...
D6.2→D6.3の四隅変更は約122g減。四隅のPLAは約44.3g増え、板と締結材の削除で約{corners['removed_metal_mass_kg']*1000:.1f}g減った。現行D6.5は金属接触防止カバー追加でD6.4から{delta_word}となり、車体推計{mass:.3f}kg。

D6.2→D6.3ではPLA材料消費参考90円増（従来基準2円/g、中実体積切上げ）、三角板素材の仮枠500円を削り、計上小計410円減だった。これは四隅だけの過去比較で、D6.4のねじ・座金・PC受けとD6.5のカバー追加は別に[BOM](BOM.ja.md)へ計上する。実スライス・サポート・失敗分は含まない。基礎のねじ・座金一式仮枠2300円は購入パック未確定のため据置き。溝ナットは70/100個使用で購入パック数は変わらない。410円は確定購入額の節約ではない。
