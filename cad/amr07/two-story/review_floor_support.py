...
for ix,iy in product(range(2),repeat=2):
    ...
    for f in fixings:
        ...
        if f['thread']=='M6':
            ...
            assert abs(b.ZMax-101.4)<1e-6
            shifted=bolt.copy();shifted.translate(V(0,0,-.01))
            area=panel.common(shifted).Volume/.01
            assert area>20,(name,f,area)
            rail_area=None
...
for ix,iy in product(range(2),repeat=2):
    ...
    tool=Part.makeCylinder(5.5,35,V(x,y,58.2)).cut(Part.makeCylinder(4,35,V(x,y,58.2)))
...
out=dict(revision='D6.2',native_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
         ...
         required_prototype_checks=['Printed dimensions, countersink seating, nut engagement and driver/wrench access.',
             ...
             'Install representative equipment and just-snug belts; dwell at expected equipment temperature and inspect panel sag, countersink damage and bolt relaxation. Set allowable belt tension from test.',
