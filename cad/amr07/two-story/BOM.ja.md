# D6.3 BOM：費用の内訳と購入リスト

**途中小計 約92,647円 ＋ 未計上分**。通常積載目標10kg、車体推計10.017kg（10kgは目安）の現行構成。
...
| モーター2個 | 14,000円 | 15.1% |
| フレーム・締結材 | 13,479円 | 14.5% |
| 専用モーター金具2個 | 12,935円 | 14.0% |
| タイヤ・キャスター | 10,413円 | 11.2% |
| 制御・電源保護 | 8,084円 | 8.7% |
| アルミ天板 | 6,078円 | 6.6% |
| PLA・ベルト・保護材 | 2,677円 | 2.9% |
| その他送料 | 1,600円 | 1.7% |
| **途中小計** | **92,647円** | **100%** |

割合の分母は計上済み小計。未計上品を0円とは扱わない。円換算は記録済みの1 USD＝157.2672円（2026-09-21参考値）で、現在の決済レートではない。元の小計は73,633円＋120.90 USD。行ごとの四捨五入で端数差が出る。
...
価格記録は既存の販売ページ確認・引継価格。自動見積はJLCCNCの実サイト記録で、未発注・担当者審査前。**モーター14,000円は仮予算**。PLA1198円は材料消費参考で、新規スプールの購入額ではない。
...
<details><summary>フレーム・締結材 — 13,479円</summary>
...
<details><summary>PLA・ベルト・保護材 — 2,677円</summary>

| 品目 | 使用／購入 | 金額 | 根拠 |
|---|---|---:|---|
| 四隅まで覆うリブ付き床4枚・継ぎ目支持梁1個・荷物ストッパ4個 | 消費599g | 1,198円 | 材料消費 |
...
**今回D6.3は四隅の平面補強板4枚と締結材24点を廃止し、床ねじ4組の位置を変更。素材仮枠500円を削除。金具・ねじの追加購入は0個、PLA材料消費参考は前版より90円増。** 各床4点固定と支柱根元両側補強を継続する。前回D6.2で追加したねじ・座金類の未計上状態は継承し、今回0円で調達できるという意味ではない。
