...
def export(data):
    ...
    overview = [
        ['円の途中小計', f'=SUMIF(\'購入品\'!H5:H{end},"JPY",\'購入品\'!G5:G{end})', 'キャスターをナフコ店舗受取498円＋送料0円とする条件。受取店・在庫未確定。販売記録・仮枠・材料消費を含む。'],
        ['USDの途中小計', f'=SUMIF(\'購入品\'!H5:H{end},"USD",\'購入品\'!G5:G{end})', '金具2個75.02＋送料7.23。自動見積・手動審査前。'],
        ['完成車総額', '未確定', '電池・主計算機・配線・基板・税・送料等が未選定／未見積。通貨未換算。'],
        ['運用の設計条件', '平坦な屋内・通常積載10kg', '追加機械ブレーキなし。構造検証は荷物15kg、SF2目標。'],
        ['車体重量の推計', 8.73314316476638, '電装枠を含むCAD推計、実測ではない。'],
        ['数量照合', 222, 'CAD物理形状を全件、費用行へ対応付け。予約形状は部品として数えない。'],
        ['小径座金の税訂正', 29, '10枚×29円は税別。税込319円へ修正し、以前の途中小計から29円増。'],
        ['価格の確認範囲', 'タイヤ・キャスター・座金・タイヤ送料を再確認', 'Amazonは再取得できず既存価格。その他は2026-09-21等の既存記録・仮枠。'],
        ...
        ['格子穴上板のCNC比較見積', '別資料、現行BOMには未加算', '300×300×4、50mmピッチ36穴。CNC品を採用する場合は板素材代と対象送料を置換し、重複加算しない。']]
    ws = sheet('概要', 'AMR-01 / A6　調達BOM', ['項目', '値', '説明'], overview,
               [29, 50, 77], '2026-09-21 | 購入一覧・締結内訳・製作品・未選定品・CAD対応を収録', 42)
    ...
    ws = sheet('製作品', '外注・自加工・3Dプリント部品', ['ID', '部品', '数量', '材質・寸法', '製作方法', '費用計上先', '設計資料', '備考'],
               ...
               [9, 27, 8, 40, 17, 25, 13, 75], '親行の素材費・加工見積に含む。モーター金具以外の工具・工賃・外注完成価格は未計上。', 50)
    ...
    sheet('CAD照合', 'CAD物理形状と費用行の対応', ['CADオブジェクト', '費用ID', 'ラベル', '材料区分', '締結仕様'],
          ...
          [40, 38, 83, 16, 55], '全222形状。1購入品＝複数CAD形状の場合があります。タイヤ付属ねじと電装予約を別購入品として数えていません。', 32)
