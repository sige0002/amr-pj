"""A3 DDSM115 nominal CAD / inventory / mass checks. Run inside FreeCAD after build.
...
doc = App.getDocument('AMR01_DDSM115_A3')
...
fixed = [o for o in parts if not o.Name.startswith(('Caster', 'Swivel', 'Tire', 'DDSMRotatingBody', 'DDSMFrontCover'))]
lowest = min(fixed, key=lambda o: bounds[o.Name].ZMin)
clearance_pairs = [('UpperMountAngleL', 'Rail400_4'), ('UpperMountAngleL', 'LowerMountAngleL'),
                   ('KeyedSteelSupportL', 'DDSMFixedBossL'), ('KeyedSteelSupportL','DDSMRotatingBodyL'),
                   ('Rail400_4', 'TireL'), ('AngleJoint_110105L','UpperMountAngleL'),
                   ('DDSMFaceBolt_0L','LowerMountAngleL')]
...
direct_mount = {}
for name, rails in [('UpperMountAngleL', ['Rail400_4']),
                    ('UpperMountAngleR', ['Rail400_1']),
...
mass = [
    ...
    {'part': 'DDSM115 complete wheels x2', 'kg': 1.530, 'basis': 'manufacturer765+/-15g each; includes tire,motor,driver,factory harness'},
...
skip_prefix = ('Rail400_', 'Cross300_', 'Bracket_', 'DDSMFixedBoss', 'DDSMRotatingBody', 'DDSMFrontCover', 'Tire')
...
step = Part.Shape()
step.read(str(HERE / 'AMR01_DDSM115_A3.step'))
...
result = {
    ...
    'drive_interfaces_mm': {'fixed_face_to_wheel_center':36.5, 'tire_width':43,
        'body_with_front_cover':50.5,'fixed_boss_projection':15,'frame_to_tire_gap':3,
        'M2p5_screw_engagement':5,'M2p5_thread_depth_max':6,
        'keyed_plate_thickness':5,'key_nominal_radial_flat_clearance':.05,
        'cable_exit_hole':10,'cable_lateral_loading_slot_width':7,
        'old_motor_face_to_wheel_center':103,'external_couplings_shafts_bearings_removed':True},
...
assert len(inventory['M4x16 socket screw']) == 12
assert len(inventory['M2.5x10 socket screw']) == 6
...
assert math.isclose(bounds['DDSMFixedBossL'].YMin,138) and math.isclose(bounds['TireL'].YMin,153)
assert math.isclose(bb.YLength,407)
assert math.isclose(doc.getObject('LowerMountAngleL').Shape.distToShape(doc.getObject('UpperMountAngleL').Shape)[0],0,abs_tol=1e-7)
